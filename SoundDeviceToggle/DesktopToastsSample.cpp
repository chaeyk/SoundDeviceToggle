...
int WINAPI wWinMain(_In_ HINSTANCE /* hInstance */, _In_opt_ HINSTANCE /* hPrevInstance */, _In_ LPWSTR /* lpCmdLine */, _In_ int /* nCmdShow */)
{
...

    {
        ...
        if (SUCCEEDED(hr))
        {
			...
			hr = SoundDeviceToggle(deviceName);
			if (SUCCEEDED(hr))
			{
				app.DisplayToast(deviceName);
				app.RunMessageLoop();
			}
        }
    }
...
DesktopToastsApp::DesktopToastsApp() : _hwnd(nullptr)
{
}
...
HRESULT DesktopToastsApp::TryCreateShortcut()
{
    ...
    if (SUCCEEDED(hr))
    {
        errno_t concatError = wcscat_s(shortcutPath, ARRAYSIZE(shortcutPath), L"\\Microsoft\\Windows\\Start Menu\\Programs\\SoundDeviceToggle.lnk");
...
HRESULT DesktopToastsApp::Initialize()
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr))
        {
                // Create window
            _hwnd = CreateWindow(
                L"SoundDeviceToggleApp",
                L"Sound Device Toggle App",
                WS_OVERLAPPEDWINDOW,
                CW_USEDEFAULT,
                CW_USEDEFAULT,
                350,
...
// COM wrappers, etc.) 
HRESULT DesktopToastsApp::DisplayToast(const std::wstring& message)
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        hr = CreateToastXml(toastStatics.Get(), message, &toastXml);
        if (SUCCEEDED(hr))
        {
...
HRESULT DesktopToastsApp::CreateToastXml(_In_ IToastNotificationManagerStatics *toastManager,
	_In_ const std::wstring& message, _Outptr_ IXmlDocument** inputXml)
{    
    ...
    {    
        ...
        if (SUCCEEDED(hr))
        {
            ...
            if (SUCCEEDED(hr))
            {
                ...
                UINT32 textLengths[] = {message.size()};

                hr = SetTextValues(textValues, 1, textLengths, *inputXml);
            }
        }
    }
    return hr;
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            if (SUCCEEDED(hr))
            {
                ...
                ComPtr<ToastEventHandler> eventHandler(new ToastEventHandler(_hwnd));
